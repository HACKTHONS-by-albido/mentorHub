...
import { useRouter } from "next/navigation";
  ...
  const InterestDropdown: React.FC<{ onInterestsChange: (interests: Option[]) => void }> = ({ onInterestsChange }) => {
    ...
    const select = (option: Option): void => {
      if (!selected.find((item) => item.id === option.id)) {
        ...
        onInterestsChange([...selected, option]);
      }
    };
  
    const remove = (id: number): void => {
      setSelected(selected.filter((item) => item.id !== id));
      onInterestsChange(selected.filter((interest) => interest.id !== id));
    };
  
    const goSearch = (): void => {
      setShowSelector(!!search);
    };
  ...
  const MentorHub: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      const locationData = location && location.split(", ");
      const [lat, lng] = locationData ? locationData : [0, 0];
      
      const avatar1 = await upload(avatar);
      
      const response = await axiosInstance.post('/adduserdata', {
          name: name,
          avatar: avatarurl,
          location: `${lat}, ${lng}`,
          interests: interests
    ...
    const handleInterestsChange = (selectedInterests: Option[]) => {
        ...
        <div className="max-w-md w-full bg-white shadow-md rounded-md overflow-hidden">
          <div className="p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <InterestDropdown  onInterestsChange={handleInterestsChange} />
              </div>
              <div>
